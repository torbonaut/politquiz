  ...
  return `
    <div class="card ${isTopCard ? 'active' : ''}"
      ...
      <div class="inner">
        ...
        <div class="front">
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border-4 border-slate-200 dark:border-slate-700 h-full flex flex-col">
            <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
              <h2 class="text-sm font-bold text-blue-600 dark:text-blue-400">Frage ${question.id}</h2>
            </div>
        ...
        <div class="back">
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border-4 border-slate-200 dark:border-slate-700 h-full flex flex-col">
                ...
                ${question.tags && question.tags.length > 0 ? `
                  <div class="flex flex-wrap gap-0.5 justify-end">
                    ${[...question.tags].sort().map(tag => `
                      <span class="text-[10px] px-1 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded">${tag}</span>
                    `).join('')}
                  </div>
                ` : ''}
              </div>
            ...
            <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-700">
              <div class="result-header text-2xl font-bold text-center px-4 py-2 rounded-lg" data-correct="${question.correctAnswer}">
                <!-- Will be filled by JavaScript -->
              </div>
            </div>

            <!-- Tabs -->
            <div class="px-4 border-b border-slate-200 dark:border-slate-700">
              <div class="flex gap-2">
                <button class="tab-button active px-4 py-2 text-sm font-medium border-b-2 border-blue-500 text-blue-600 dark:text-blue-400" data-tab="answer">
                  Antwort
                </button>
                <button class="tab-button px-4 py-2 text-sm font-medium border-b-2 border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100" data-tab="info">
                  Hintergrund & Quellen
                </button>
              </div>
            </div>
            ...
            <div class="content flex-1 overflow-y-auto px-4 py-3">
              <!-- Tab 1: Answer -->
              <div class="tab-content active" data-tab-content="answer">
                <h3 class="text-base font-semibold mb-4 text-slate-900 dark:text-slate-100">${question.question}</h3>
                ...
                <div class="mb-4 space-y-2 bg-slate-100 dark:bg-slate-700/50 p-3 rounded-lg">
                  <p class="text-slate-900 dark:text-slate-100 text-sm">
                    <strong>Richtige Antwort:</strong> ${question.correctAnswer}) ${question.options.find(o => o.letter === question.correctAnswer)?.text}
                  </p>
              ...
              <!-- Tab 2: Background Info & Sources -->
              <div class="tab-content hidden" data-tab-content="info">
                <div class="mb-3 bg-blue-50 dark:bg-slate-700/30 p-2.5 rounded-lg border border-blue-200 dark:border-slate-600">
                  <h3 class="text-xs font-semibold mb-1.5 text-blue-700 dark:text-blue-400">Hintergrundinformation</h3>
